...
export async function insertDiagnostics(records) {
  ...
  for (let i = 0; i < normalized.length; i += CHUNK) {
    ...
    const { error } = await supabase
      .from('店舗診断表')
      .upsert(chunk, {
        // ③ 重複判定キー（テーブル側で UNIQUE 制約を張ってください）
        onConflict: '店舗名,月,項目',
    ...
    if (error) {
      console.error('Supabase への upsert に失敗:', error);
      throw error; // 失敗した時点で呼び出し側へ
    }
  }
}
...
function updateNotificationUI() {
  ...
  if (perm === "granted") {
    ...
    badge.textContent = "拒否中";
    badge.className = "badge rounded-pill bg-danger";
    btn.textContent = "ブラウザ設定で許可に変更";
    btn.classList.remove("btn-primary");
    btn.classList.add("btn-outline-secondary");
    btn.disabled = true;
  } else {
...
async function requestNotificationPermission() {
  ...
  try {
    ...
    console.error("通知許可の要求に失敗:", e);
  } finally {
    updateNotificationUI();
  }
}

// どこかの初期化処理（DOMContentLoadedなど）で一度呼ぶ
window.addEventListener("DOMContentLoaded", () => {
  updateNotificationUI();
});

// 既存どおり window に公開しているなら必要に応じて
